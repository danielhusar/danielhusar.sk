---
url: 'reducing-the-intercom-messenger-bundle-size-by-65-percentages'
date: '2019-01-20'
title: 'Reducing the Intercom Messenger bundle size by 65%'
banner: './how-we-reduced-messenger-size/banner.png'
draft: false
---

_[This is a cross post from the Intercom blog.](https://www.intercom.com/blog/reducing-intercom-messenger-bundle-size/)_
...
export const setLocale = async (locale) => {
  if (!defaultLocales[locale]) {
    const data = await import(/* webpackChunkName: "locale-[request]" */ `intercom-js-translations/${locale}.json`)
...
Sentry.init({
  ...
  integrations: (integrations) => integrations.filter((integration) => integration.name !== 'GlobalHandlers'),
})
...
export const logError = async (error) => {
  const { captureException } = await import(/* webpackChunkName: "sentry" */ './sentry')
